...
/*********************************************************************************************************
** 函数名称: __tshellCmdMatchPart
** 功能描述: 在 ttiny shell 系统查找一个不完整的关键字，如果存在则输出关键字和参数列表.
** 输　入  : pcKeyword     关键字
**           pskwNode     关键字数组
** 输　出  : 匹配到的关键字数量
** 全局变量:
...
VOID __tshellCmdMatchPart(INT iFd, CPCHAR  pcKey, __PTSHELL_KEYWORD pskwNode[])
{
    ...
    do {
        ...
        if (ulGetNum) {
            ...
            for (i=0; i<100; i++){
                if (pskwNode[0]->SK_pcKeyword[i] == '\0'){
                     wlen = i;
                     break;
                }
           }
            ...
            if (klen <= wlen){
                ...
                i = 0;
                while (i < wlen && j < klen){
                    if (pcKey[j] == pskwNode[0]->SK_pcKeyword[i]){
                        i++;
                        j++;
                    }
                    else{
                ...
                if (j == klen){
                    __helpPrint(pskwNode[0], LW_FALSE);
                }
            }
    ...
    CHAR cStat[MAX_FILENAME_LENGTH];
    size_t   stCatLen;
    snprintf(cStat, MAX_FILENAME_LENGTH, "%s", pcKey);
    stCatLen = lib_strlen(cStat);
    write(iFd, cStat, stCatLen);
    return;
}
//SysCmd.c里面的helpPrintKeyword()
