...
** 调用模块:
*********************************************************************************************************/
static INT  __tshellFsCmdHead (INT  iArgC, PCHAR  ppcArgV[])
{
             BOOL           bLastLf = LW_TRUE;
    REGISTER INT            iError;
    REGISTER ssize_t        sstNum;
    ...
    iFd = open(ppcArgV[rntNum], O_RDONLY, 0);
    if (iFd < 0) {
        if (errno == EACCES) {
            fprintf(stderr, "insufficient permissions.\n");
        } else {
    ...
    if(iArgC == 4) {
        if(lib_strcmp(ppcArgV[1], "-n")) {
            fprintf(stdout,"unrecognized parameter %s", ppcArgV[1]);
        } else {
            sscanf( ppcArgV[2], "%d", &Tline);
        }
    }
    ...
    do {
        ...
        sstNum = 0;
        do {
            rntNum = read(iFd, rBuffer, 1);
            sstNum += rntNum;
            lib_strlcat(cBuffer, rBuffer, MAX_FILENAME_LENGTH);
        }while(rBuffer[0]!='\n' && rntNum!=0);
        fprintf(stdout, "%s", cBuffer);
        bLastLf = (cBuffer[sstNum] == '\n') ? LW_TRUE : LW_FALSE;
        NumLine++;
    } while (sstNum > 0 && NumLine < Tline);
...
static INT  __tshellFsCmdSed (INT  iArgC, PCHAR  ppcArgV[])
{
    ...
    REGISTER INT            iFd;
             ...
             INT            stCatLen;
             INT            stCatLen2;
             CHAR           OpBuffer1[MAX_FILENAME_LENGTH];
             CHAR           OpBuffer2[MAX_FILENAME_LENGTH];
             CHAR           OpBuffer3[MAX_FILENAME_LENGTH];
             PCHAR          first;
             PCHAR          second;
             INT            ARG1;
             INT            ARG2;
    ...
    if(iArgC == 2) {
        ...
        iFd = open(ppcArgV[2], O_RDONLY, 0);
    } else {
        iFd = open(ppcArgV[3], O_RDONLY, 0);
    }
    ...
    if(iArgC == 2) {
        ...
        stCatLen = lib_strlen(ppcArgV[1]);
        lib_strcpy(OpBuffer1,ppcArgV[1]);
        lib_strcpy(OpBuffer2,OpBuffer1);
        OpBuffer1[stCatLen] = PX_EOS;
        OpBuffer2[stCatLen] = PX_EOS;
        stCatLen2 = lib_strlen(OpBuffer1);
        ...
        if(stCatLen==1) {
            ...
            if (cnt == stCatLen-1) {
                ...
                // 有两个行号
                OpBuffer2[stCatLen-1] = '\0';
                second = &OpBuffer2[cnt+1];
                OpBuffer2[cnt] = '\0';
                first = OpBuffer2;
                sscanf(first, "%d",  &ARG1);
                sscanf(second, "%d",  &ARG2);
            }
        }

        if(ARG1>ARG2) {
            fprintf(stderr, "command error: please enter the correct line number!\n");
            return  (-ERROR_TSHELL_EPARAM);
        }

        if(OpBuffer1[stCatLen-1] == 'a') {
            if(iArgC == 3){
                fprintf(stderr, "please add the content after operation command!\n");
                return  (-ERROR_TSHELL_EPARAM);
            }
            lib_strcpy(OpBuffer2,ppcArgV[2]);
            LineNum = 1;
            do {
                cBuffer[0] = PX_EOS;
                sstNum = 0;
                ...
                do {
                    ...
                    if(LineNum>=ARG1 && LineNum <=ARG2) {
                        cBuffer[sstNum] = PX_EOS;
                        lib_strlcat(cBuffer, OpBuffer2, MAX_FILENAME_LENGTH);
                        fprintf(stdout,"%s\n", cBuffer);
                        LineNum ++;
                        continue;
                    } else {
             ...
             return  (ERROR_NONE);
        } else if(OpBuffer1[stCatLen-1] == 'c') {
            if(iArgC == 3){
                            fprintf(stderr, "please add the content after operation command!\n");
                            return  (-ERROR_TSHELL_EPARAM);
                        }
                        ...
                        do {
                         ...
                         return  (ERROR_NONE);
        } else if (OpBuffer1[stCatLen-1] == 'd') {
            LineNum = 1;
            do {
                cBuffer[0] = PX_EOS;
                sstNum = 0;
            ...
            return  (ERROR_NONE);
        } else if (OpBuffer1[stCatLen-1] == 'i') {
            if(iArgC == 3){
                fprintf(stderr, "please add the content after operation command!\n");
                return  (-ERROR_TSHELL_EPARAM);
            }
            ...
            do {
                ...
                do {
                    ...
                    if(LineNum>=ARG1 && LineNum <=ARG2) {
                        OpBuffer3[0] = PX_EOS;
                        lib_strlcat(OpBuffer3, OpBuffer2, MAX_FILENAME_LENGTH);
                        lib_strlcat(OpBuffer3, cBuffer, MAX_FILENAME_LENGTH);
                        fprintf(stdout,"%s", OpBuffer3);
                        LineNum ++;
                        continue;
                    } else {
                        fprintf(stdout,"%s", cBuffer);
             ...
             return  (ERROR_NONE);
        } else if (OpBuffer1[stCatLen-1] == 'p') {
            printf("operation p under develope..\n");
        } else if (OpBuffer1[stCatLen-1] == 's') {
            printf("operation s under develope..\n");
        } else {
            fprintf(stderr, "unkown operation %s!\n", &OpBuffer1[stCatLen-1]);
            close(iFd);
            return  (-ERROR_TSHELL_EPARAM);;
        }
    }
...
VOID  __tshellFsCmdInit (VOID)
{
    ...
    API_TShellKeywordAdd("head", __tshellFsCmdHead);
    API_TShellFormatAdd("head", " file name");
    API_TShellHelpAdd("head", "display the first n line in the file\n");

    API_TShellKeywordAdd("sed", __tshellFsCmdSed);
    API_TShellFormatAdd("sed", " [operation][content] file name");
    API_TShellHelpAdd("sed", "a fake sed, only changed the content in stdout but not the real file \n"
